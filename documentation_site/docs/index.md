...
2. **Strict entity referencing**: Flows, methods and properties can only use other flows, methods or properties they explicitly refer to by their identifier.

### Strict entity referencing

_Strict entity referencing_ is different from _strict typing_ to the extent that not only the type of the entity has to match, also its identifier has to match.

In traditional _strict typing_ environment values are matched by its type. In the following example a method `greet` accepts any string.
...
We cannot determine just by looking at the code whether this was intentional, an error, or the desired outcome.
...
_Execution Flows_ paradigm brings a lot of boilerplate code, that;s for sure. But the value it brings in large projects is significantly higher.

For example, tools like `mypy` provide much more value when combined with _strict entity referencing_. 

Now imagine a large project with hundreds of flows, properties, and methods. Consider how much more robust the entire codebase becomes due to the _strict entity referencing_.
