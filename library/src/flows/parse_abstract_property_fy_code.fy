...
flow ParseAbstractPropertyFyCode:
    ...
    def -> ParsedFyPyFile:
        ...
        assert len(
            abstract_property_file_split
        ) == 4, f"Abstract property file split length {len(abstract_property_file_split)} is invalid"

        abstract_property_name = PythonEntityName.from_snake_case(abstract_property_file_split[1])
